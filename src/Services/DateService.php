...
class DateService implements DateCalculationInterface {
		...
		while (true) {
			...
			if ($arretDroits === 0) {
				...
				if ((isset($currentData['dt-line']) && $currentData['dt-line'] == '0') && !empty($currentData['declaration-date-line'])) {
					$choice = $currentData; // Default to current data
					if ($increment > 0) {
						$choice = $arrets[0];
					}
					$slect = $choice;
				...
				if ($siRechute) {
					...
					if ((isset($currentData['dt-line']) && $currentData['dt-line'] == '0') && !empty($currentData['declaration-date-line'])) {
						...
						$slect = (isset($choice)) ? $choice : $currentData;
						$dtDate = new DateTime($slect['declaration-date-line']);
	...
	/**
	 * @param array<string, mixed> $previousArret
	 * @param array<string, mixed> $currentData
	 * @param bool $siRechute
	 * @return bool
	 */
	private function isRechute(array $previousArret, array $currentData, bool $siRechute = false): bool {
		...
		foreach ($arrets as $index => $arret) {
			...
			if (isset($arret['cco_a_jour']) && $arret['cco_a_jour'] != 1) {
				...
				$paymentDetails[$index] = [
					...
					'is_rechute' => $arret['is_rechute'] ?? false,
					'rechute_of_arret_index' => $arret['rechute_of_arret_index'] ?? null,
	 ...
	 *
	 * @param array<string, mixed> $arret L'arrêt à analyser
