...
class DateNormalizer
{
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $key => $value) {
                ...
                if (in_array($key, self::DATE_FIELDS)) {
                    $normalized[$key] = self::normalizeDate($value);
                } else {
     ...
     *
     * @param array|null $arrets Arrets data
     * @return array|null Normalized arrets
     */
    public static function normalizeArrets(?array $arrets): ?array
    {
        ...
        if (isset($arrets['arret-from-line'])) {
            // Single arret
            return self::normalize($arrets);
        }

        // Array of arrets
        $normalized = [];
        foreach ($arrets as $index => $arret) {
            $normalized[$index] = self::normalize($arret);
        }
